...
import time
from .NequickG import Position, GalileoBroadcast, NEQTime, NequickG, NequickGparameters
from .spheretrig import *


class NequickGGlobal:
    ...
    def get_nequick_local(self, position):
        Para = NequickGparameters(position, self.broadcast, self.time)
    ...
    def slant_table(self, n, ray, path, ex_attr=None):
        ...
        allowed_attrs = set(['foF1', 'foF2', 'foE', 'M3000F2', 'NmF2', 'NmF1', 'NmE', 'hmE', 'hmF1', 'hmF2', 'modip', 'Az', 'Azr', 'solarsine',
                             'solarcosine', 'chi', 'chi_eff', 'H0', 'B1bot', 'B1top', 'B2bot', 'BEtop', 'BEbot', 'A1', 'A2', 'A3', 'k', 'seasp'])
    ...
    def s_tec_calculate(self, ray, tolerance=None):
        """
        ...
        if ray.p_radius < 0.1:
            print("calulcating v_tec_calculate instead")
            pos = Position(ray.ob_lat, ray.ob_lon)
            neq, para = self.get_nequick_local(pos)
            return neq.v_tec_calculate(ray.ob_h, ray.sat_h)
    ...
    def s_tec2_calculate(self, ray):
        ...
        stec1 = self.s_tec_calculate(ray1, tolerance=0.001)
        stec2 = self.s_tec_calculate(ray2, tolerance=0.01)

        return stec1 + stec2

    def map_v_tec(self, lat1, lon1, lat2, lon2, resolution=40):
        ...
        for i in range(resolution):
            for j in range(resolution):
                ...
                neq, para = self.get_nequick_local(pos)
                vtec[i, j] = neq.v_tec_calculate(100, 20000)
            ...
            for j in range(resolution):
                ...
                for k in range(len(attrs)):
                    ...
                    try:
                        ...
                        if attrs[k] == 'v_tec_calculate':
                            out[i, j] = neq.v_tec_calculate(100, 20000)
                        else:
...
class Ray:
    ...
    def __init__(self, h1, lat1, lon1, h2, lat2, lon2):
        ...
        self.ob_zenith = zenith(h1, lat1, lon1, h2, lat2, lon2)
        self.ob_azimuth = azimuth(lat1, lon1, lat2, lon2)
        self.greatcircle = greatcircle(lat1, lon1, lat2, lon2)
    ...
    def perigee_azimuth(self):
        ...
        if abs(abs(self.p_lat) - 90) < 10 ** -10 * 180 / np.pi:
            ...
            return azimuth(self.p_lat, self.p_lon, self.sat_lat, self.sat_lon)
...
def gaussquadrature2_segment(n, x1, x2):
    ...
    xx = np.empty(2 * n)
    I_v = np.arange(n)
    xx[0::2] = x_y + I_v * deltax
    xx[1::2] = x_y + I_v * deltax + x_g
